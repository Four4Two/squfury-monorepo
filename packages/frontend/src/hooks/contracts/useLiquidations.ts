...
import { liquidations } from '../../queries/squfury/__generated__/liquidations'
import { LIQUIDATIONS_QUERY } from '../../queries/squfury/liquidationsQuery'
...
export const useVaultLiquidations = (vaultId: number /* refetchIntervalSec = 30 */) => {
  ...
  useAppEffect(() => {
    ...
    const _liquidations = data?.liquidations.map((l) => {
      return {
        ...l,
        debtAmount: toTokenAmount(l.debtAmount.toString(), OSQUFURY_DECIMALS),
