...
export default function LongSquFury() {
  const classes = useStyles()
  const { loading: isPositionLoading } = useLPPositionsQuery()
  const { squfuryAmount, loading: swapsLoading } = useComputeSwaps()
  ...
  return (
    <div className={classes.position}>
      <div className={classes.positionTitle}>
        <Typography>Long SquFury</Typography>
      </div>
        ...
        <div className={classes.innerPositionData}>
          <div style={{ width: '50%' }}>
            ...
            <Typography variant="body1">
              {isPositionLoading && squfuryAmount.isEqualTo(0) ? (
                'Loading'
              ) : (
                <span id="pos-page-long-osqfu-bal">{squfuryAmount.toFixed(8)}</span>
              )}{' '}
              &nbsp; oSQFU
            </Typography>
          </div>
