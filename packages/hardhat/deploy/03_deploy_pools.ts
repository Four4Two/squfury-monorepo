...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const squfuryPriceInEth = 1300 / oracleScaleFactor.toNumber();
  const squfuryWethPool = await createUniPool(squfuryPriceInEth, weth9, squfury, positionManager, uniswapFactory)
  const tx1 = await squfuryWethPool.increaseObservationCardinalityNext(128)
  await ethers.provider.waitForTransaction(tx1.hash, 1)

  console.log(`SQF/ETH Pool created 🐑. Address: ${squfuryWethPool.address}`)
