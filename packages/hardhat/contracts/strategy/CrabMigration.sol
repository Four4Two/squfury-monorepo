...
contract CrabMigration is Ownable {
    ...
    function _flashCallback(
        ...
        if (FLASH_SOURCE(_callSource) == FLASH_SOURCE.BATCH_MIGRATE) {
            ...
            // 2. mint osqfu in crab V2
            uint256 wSquFuryToMint = CrabStrategy(crabV1).getWsqufuryFromCrabAmount(crabV1Balance);
            ...
            IERC20(wPowerPerp).approve(crabV1, data.v1oSqfuToPay);
            ...
            uint256 oSqfuToPay = IERC20(wPowerPerp).balanceOf(address(this));
            IERC20(wPowerPerp).approve(crabV1, oSqfuToPay);

            // find crab amount for contract's sqfu balance
            // remaining crab can be withdrawn using flash withdraw
            uint256 crabV1ToWithdrawRmul = oSqfuToPay.wmul(CrabStrategy(crabV1).totalSupply()).rdiv(v1Short);
    ...
    function flashMigrateFromV1toV2(
        ...
        (bool isFlashOnlyMigrate, uint256 ethNeededForV2, uint256 v1oSqfuToPay, ) = _flashMigrationDetails(_v1Shares);
     ...
     * @param _withdrawMaxEthToPay maximum ETH to pay to buy back the owed wSquFury debt
    ...
    function flashMigrateAndWithdrawFromV1toV2(
        ...
        (bool isFlashOnlyMigrate, , uint256 v1oSqfuToPay, ) = _flashMigrationDetails(_v1Shares);
        ...
        )
    {
        ...
        uint256 v1oSqfuToPay = v1TotalShort.wmul(_v1Shares).wdiv(CrabStrategy(crabV1).totalSupply());
        uint256 ethNeededForV2 = v1oSqfuToPay.wmul(v2TotalCollateral).rdiv(v2TotalShort).ceil(10**9) / (10**9);
        uint256 ethToGetFromV1 = _v1Shares.wdiv(CrabStrategy(crabV1).totalSupply()).wmul(v1TotalCollateral);

        return (ethNeededForV2 <= ethToGetFromV1, ethNeededForV2, v1oSqfuToPay, ethToGetFromV1);
    }
