...
// Example execution
/**
 npx hardhat buySquFury --input '0.1'
 */
task("buySquFury", "Buy SquFury from the pool")
  ...
  console.log(squfury.address, 'squfury')
  let wethBalance = await weth.balanceOf(deployer)
  
  const squfuryBalance = await squfury.balanceOf(deployer)
  ...
  console.log(`SQF Balance before trade:\t${ethers.utils.formatUnits(squfuryBalance.toString())}`)  
  ...
  const exactInputParam = {
    ...
    tokenOut: squfury.address, // address
    fee: 3000, // uint24
  ...
  const squfuryBalanceAfter = await squfury.balanceOf(deployer)
  const wethBalanceAfter = await weth.balanceOf(deployer)

  console.log(`Bought ${ethers.utils.formatUnits(squfuryBalanceAfter.sub(squfuryBalance))} SQF from Uni Pool`)
