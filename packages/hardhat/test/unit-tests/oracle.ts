...
describe("Oracle", function () {
  const squfuryPriceInETH = 2000; // can sell 1 squfury for 2000 eth
  let initPriceTick: number
  let isWethToken0: boolean

  const squfuryPriceInETH1e18 = (squfuryPriceInETH * 1e18).toString()
  ...
  this.beforeAll("Deploy uniswap protocol & setup uniswap pool", async() => {
    ...
    // Create ETH/SQUFURY Pool with positionManager
    squfury = (await (await ethers.getContractFactory("MockWPowerPerp")).deploy()) as MockWPowerPerp;
    isWethToken0 = parseInt(weth.address, 16) < parseInt(squfury.address, 16)
    

    const sqrtX96Price = isWethToken0 
      ? convertToken0PriceToSqrtX96Price((new BigNumberJs(1).div(squfuryPriceInETH)).toString()).toFixed(0)
      : convertToken0PriceToSqrtX96Price(squfuryPriceInETH.toString()).toFixed(0)
    ...
    // set pool
    const squfuryPoolAddr = await uniswapFactory.getPool(token0, token1, 3000)
    squfuryPool = await ethers.getContractAt("IUniswapV3Pool", squfuryPoolAddr);
    initPriceTick = (await squfuryPool.slot0()).tick
    const cardinality = (await squfuryPool.slot0()).observationCardinality
  ...
  describe("Fetch price right after initialization", async () => {
    it("should return initial price with period = 1", async () => {
      const price = new BigNumberJs((await oracle.getTwap(squfuryPool.address, squfury.address, weth.address, 1, false)).toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it('should be able to get TWAP since init time', async() => {
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSince(interactionTimestamps[0], squfuryPool.address, squfury.address, weth.address))
        .toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    ...
    it("should revert if trying to request twap since a time before initialization", async () => {
      await expect(
        oracleTester.testGetTwapSince(interactionTimestamps[0] - 1, squfuryPool.address, squfury.address, weth.address)
    ...
    it("should NOT revert if trying to request twap since a time before initialization, with #getTwapSafe", async () => {
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSafeSince(interactionTimestamps[0] - 1, squfuryPool.address, squfury.address, weth.address)
      ).toString());
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it('should return max period', async() => {
      const period = await oracle.getMaxPeriod(squfuryPool.address)
  ...
  describe("Fetch price after touching the pool", async () => {
    before('add liquidity to the pool', async() => {
      const squfuryAmount = 1
      const wethAmount = squfuryPriceInETH * squfuryAmount

      const liquiditySquFuryAmount = ethers.utils.parseEther(squfuryAmount.toString())
      const liquidityWethAmount = ethers.utils.parseEther(wethAmount.toString())

      await squfury.mint(deployer, liquiditySquFuryAmount)
      ...
      const mintParam = {
        ...
        amount0Desired: isWethToken0 ? liquidityWethAmount : liquiditySquFuryAmount,
        amount1Desired: isWethToken0 ? liquiditySquFuryAmount : liquidityWethAmount,
        amount0Min: 0,
        amount1Min: 0,
    ...
    it('should revert if requesting TWAP from init timestamp', async() => {
      await expect(
        oracleTester.testGetTwapSince(interactionTimestamps[0], squfuryPool.address, squfury.address, weth.address)
      ).to.be.revertedWith("OLD");
    })
    // todo: Fix this!
    ...
    it("should be fix in the future: if first observation is updated in the same block, the max duration will be 0 causing the library to revert.", async () => {
      await expect(
        oracleTester.testGetTwapSafeSince(interactionTimestamps[0], squfuryPool.address, squfury.address, weth.address)
    ...
    it("should NOT revert if requesting TWAP from init timestamp, with #getTwapSafe", async () => {
      ...
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSafeSince(interactionTimestamps[0], squfuryPool.address, squfury.address, weth.address)
      ).toString());
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it("should NOT revert if requesting time weighted tick from init timestamp, with #getWeightedTickSafe", async () => {
      const tick = await oracleTester.testGetWeightedTickSafe(interactionTimestamps[0], squfuryPool.address, )
    ...
    it('should be able to get TWAP since last touch', async() => {
      ...
      await provider.send("evm_mine", [])
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSince(interactionTimestamps[1], squfuryPool.address, squfury.address, weth.address))
        .toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it("should revert if trying to request twap since a time before last touch", async () => {
      await expect(
        oracleTester.testGetTwapSince(interactionTimestamps[1] - 1, squfuryPool.address, squfury.address, weth.address)
      ).to.be.revertedWith("OLD");
    })
    it("should NOT revert if trying to request twap since a time before last touch, with #getTwapSafe", async () => {
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSafeSince(interactionTimestamps[1] - 1, squfuryPool.address, squfury.address, weth.address)
      ).toString());
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it("should NOT revert if trying to request average tick since a time before last touch, with #testGetWeightedTickSafe", async () => {
      const tick = await oracleTester.testGetWeightedTickSafe(interactionTimestamps[1] - 1, squfuryPool.address)
      expect(initPriceTick === tick).to.be.true;
    })
  ...
  describe("Fetch price after adding storage slot", async () => {
    before('increase storage slot', async() => {
      ...
      await squfuryPool.increaseObservationCardinalityNext(16)
      await provider.send("evm_mine", [])
    })
    before('add liquidity to the pool', async() => {
      const squfuryAmount = 1
    ...
    it('should be able to get TWAP since second touch', async() => {
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSince(interactionTimestamps[2], squfuryPool.address, squfury.address, weth.address))
        .toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    it("should be able to get time weighted tick from second touch, with #getWeightedTickSafe", async () => {
      const tick = await oracleTester.testGetWeightedTickSafe(interactionTimestamps[2], squfuryPool.address, )
    ...
    it('should be able to get TWAP since time between first touch and second touch with #getTwapSafe', async() => {
      ...
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSafeSince(period, squfuryPool.address, squfury.address, weth.address))
        .toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
    ...
    it('should be able to get a historical TWAP from now until first interaction and it should match a normal twap', async() => {
      const price = new BigNumberJs((
        await oracleTester.testGetHistoricalTwapToNow(interactionTimestamps[1], squfuryPool.address, squfury.address, weth.address))
        .toString())
    ...
    it('should be able to get a historical TWAP from second interaction to first interaction', async() => {
      const price = new BigNumberJs((
        await oracleTester.testGetHistoricalTwap(interactionTimestamps[1], interactionTimestamps[2], squfuryPool.address, squfury.address, weth.address))
        .toString())
  
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18, 3)).to.be.true;
    })

    it('should be able to get TWAP since second touch with #getTwapSafe', async() => {
      const price = new BigNumberJs((
        await oracleTester.testGetTwapSafeSince(interactionTimestamps[2], squfuryPool.address, squfury.address, weth.address))
        .toString())
      expect(isSimilar(price.toString(), squfuryPriceInETH1e18)).to.be.true;
    })
  })
  ...
  describe("oracle tick is time weighted", async () => {
    ...
    before('setup time', async() => {
      const { tick } = await squfuryPool.slot0()
      oldTick = tick
    ...
    it('tick should return the average when each price has same weight ', async() => {
      ...
      const timeWeightedTick = await oracle.getTimeWeightedAverageTickSafe(squfuryPool.address, period*2)
      ...
      const twap = await oracle.getTwap(squfuryPool.address, squfury.address, weth.address, period*2, false)
      const twapSafe = await oracle.getTwap(squfuryPool.address, squfury.address, weth.address, period*2, true)
    ...
    it('tick should return the weighed average when price does not have same weight', async() => {
      ...
      const timeWeightedTick = await oracle.getTimeWeightedAverageTickSafe(squfuryPool.address, period*3)
      expect(isSimilar(timeWeightedTick.toString(), ((newTick*2 + oldTick) / 3).toString())).to.be.true

      // check price returned by oracle match the tick
      const twap = await oracle.getTwap(squfuryPool.address, squfury.address, weth.address, period*3, false)
      const twapSafe = await oracle.getTwap(squfuryPool.address, squfury.address, weth.address, period*3, true)
      const tickToUse = isWethToken0 ? -timeWeightedTick : timeWeightedTick
      const expectedTwap = tickToPrice1e18(tickToUse)
