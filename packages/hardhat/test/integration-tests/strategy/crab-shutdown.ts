...
describe("Crab integration test: Shutdown of SquFury Power Perp contracts", function () {
  ...
  describe("Shutdown core contracts and check results", async () => {
    ...
    it("should NOT let user withdrawShutdown pre shutdown, pre redeemShortShutdown", async () => {
        ...
        await expect(crabStrategy.connect(depositor).withdrawShutdown(userCrabBalanceBefore)).to.be.revertedWith("SquFury contracts not shut down")
      })
