...
describe("Crab Migration", function () {
    ...
    let oracle: Oracle;
    let oSqfu: WPowerPerp;
    let squfuryPool: IUniswapV3Pool;
    ...
    const squfuryControllerAddress = "0x64187ae08781B09368e6253F9E94951243A493D5";
    const oracleAddress = "0x65D66c76447ccB45dAf1e8044e918fA786A483A1";
    const uniswapFactoryAddress = "0x1F98431c8aD98523631AE4a59f267346ea31F984";
    const wethOsqfuPoolAddress = "0x82c427AdFDf2d245Ec51D8046b41c4ee87F0d29C";
    const squfuryAddress = "0xf1b99e3e573a1a9c5e6b2ce818b617f0e664e86b";
    ...
    this.beforeAll("Setup environment", async () => {
        ...
        controller = await ethers.getContractAt("Controller", squfuryControllerAddress);
        oracle = await ethers.getContractAt("Oracle", oracleAddress);
        oSqfu = await ethers.getContractAt("WPowerPerp", squfuryAddress);
        squfuryPool = await ethers.getContractAt("IUniswapV3Pool", wethOsqfuPoolAddress);

        squfuryPoolFee = BigNumber.from(await squfuryPool.fee())
    ...
    this.beforeAll("Deploy Crab 2", async () => {
        ...
        crabStrategyV2 = (await CrabContract.deploy(
            squfuryControllerAddress,
            oracleAddress,
            wethAddress,
            uniswapFactoryAddress,
            wethOsqfuPoolAddress,
            timelock.address,
    ...
    const getV2SqfuAndEth = async (share: BigNumber) => {
        const [, , eth, sqfu] = await crabStrategyV2.getVaultDetails();
        const supply = await crabStrategyV2.totalSupply();
        const userEth = wdiv(wmul(share, eth), supply)
        const userSqfu = wdiv(wmul(share, sqfu), supply)

        return [userEth, userSqfu]
    }
    ...
    describe("Test Migration", () => {
        ...
        it("Should not be able to flash migrate until strategy is migrated", async () => {
            await expect(crabMigration.connect(d3).flashMigrateFromV1toV2(0, await crabStrategyV1.balanceOf(d3.address), squfuryPoolFee)).to.be.revertedWith("M2")
        })
        ...
        it("batch migrate", async () => {
            ...
            // 4. check that oSqfu amt minted in crab v1 matches oSqfu amt minted in crab v2
            const oSqfuDebtPaidCrabV1 = crabV1VaultDetailsBefore[3].sub(crabV1VaultDetailsAfter[3])
            const oSqfuMintedCrabV2 = crabV2VaultDetailsAfter[3].sub(crabV2VaultDetailsBefore[3]);

            expect(oSqfuDebtPaidCrabV1).to.be.equal(oSqfuMintedCrabV2);
        ...
        it("Should not able to claim more than their share", async () => {
            ...
            await expect(crabMigration.connect(d2).claimAndWithdraw(d2sharesV2ToMigrate, ethers.constants.MaxUint256, squfuryPoolFee)).to.be.revertedWith("M5") // Set ETH slippage higher!
        })
        ...
        it("d2 Claim and flash withdraw 50 percent of tokens", async () => {
            ...
            const [ethToGet, sqfuToSell] = await getV2SqfuAndEth(d2sharesV2ToMigrate);
            const sqfuPrice = await oracle.getTwap(wethOsqfuPoolAddress, squfuryAddress, wethAddress, 1, false)

            const ethSpentToBuyBack = wdiv(wmul(wmul(sqfuToSell, sqfuPrice), BigNumber.from(105)), BigNumber.from(100))

            await crabMigration.connect(d2).claimAndWithdraw(d2sharesV2ToMigrate, ethSpentToBuyBack, squfuryPoolFee) // Set ETH slippage higher!
    ...
    describe("Individual claim after migration", () => {
        it("Should fail if wrong migration function is called", async () => {
            ...
            await expect(crabMigration.connect(d4).flashMigrateAndWithdrawFromV1toV2(one, one, 0, one, squfuryPoolFee)).to.be.revertedWith("M10");
            await decreaseCR1(ethers.utils.parseEther('10'))
            await initialize(d5.address)
            await decreaseCR1(ethers.utils.parseEther('10'))
            await expect(crabMigration.connect(d5).flashMigrateFromV1toV2(one, 0, squfuryPoolFee)).to.be.revertedWith("M9");
            await increaseCR1(ethers.utils.parseEther('10'))
        })

        it("Should fail if _ethToBorrow or _withdrawMaxEthToPay is passed as 0", async () => {
            await decreaseCR1(ethers.utils.parseEther('10'))
            await expect(crabMigration.connect(d5).flashMigrateAndWithdrawFromV1toV2(one, one, 0, 0, squfuryPoolFee)).to.be.revertedWith("M8");
            await expect(crabMigration.connect(d5).flashMigrateAndWithdrawFromV1toV2(one, one, one, 0, squfuryPoolFee)).to.be.revertedWith("M8");
            await expect(crabMigration.connect(d5).flashMigrateAndWithdrawFromV1toV2(one, one, 0, one, squfuryPoolFee)).to.be.revertedWith("M8");
            await increaseCR1(ethers.utils.parseEther('10'))
        })
        ...
        it("Should migrate d4 when CR1 > CR2", async () => {
            ...
            const tx2 = await crabMigration.connect(d4).flashMigrateFromV1toV2(crabV1SharesBefore, 0, squfuryPoolFee);
            ...
            const squfuryBalance = await oSqfu.balanceOf(crabMigration.address)
            ...
            expect(squfuryBalance).to.be.equal('0')
        })
        ...
        it("Should migrate d5 when CR1 < CR2", async () => {
            ...
            await initialize(d5.address)
            await decreaseCR1(ethers.utils.parseEther('10'))
            const [isFlashMigrate, ethNeededForV2, v1oSqfuToPay, ethToGetFromV1] = await crabMigration.flashMigrationDetails(crabV1SharesBefore)
            ...
            const oSqfuPrice = await oracle.getTwap(wethOsqfuPoolAddress, squfuryAddress, wethAddress, 1, false)

            const numerator = ethToGetFromV1.sub(wmul(v1oSqfuToPay, oSqfuPrice).mul(101).div(100)) // 1% slippage
            const denominator = one.sub(wdiv(oSqfuPrice.mul(101).div(100), wdiv(collatV2, shortV2)))
            const ethToFlashLoan = wdiv(numerator, denominator)
            const expectedSqfuFromV2 = wdiv(wmul(ethToFlashLoan, shortV2), collatV2)
            const v1SqfuToFlashWithdraw = v1oSqfuToPay.sub(expectedSqfuFromV2)
            const maxEthToPay = wmul(v1SqfuToFlashWithdraw, oSqfuPrice).mul(1005).div(1000) // .1% slippage
            const ethToGetFromWithdraw = wdiv(wmul(expectedSqfuFromV2, collatV1), shortV1)
            const ethToGetFromFlashWithdraw = wdiv(wmul(v1SqfuToFlashWithdraw, collatV1), shortV1).sub(maxEthToPay)
            ...
            const tx2 = await crabMigration.connect(d5).flashMigrateAndWithdrawFromV1toV2(crabV1SharesBefore, 0, ethToFlashLoan, maxEthToPay, squfuryPoolFee)
            gasPaid = gasPaid.add(await getGasPaid(tx2))
        ...
        describe("Migrate crab with even supply", async function () {
            ...
            this.beforeAll("Deploy and seed crab V1", async () => {
                ...
                crabStrategyV1 = (await CrabStrategyContract.deploy(controller.address, oracle.address, weth.address, uniswapFactoryAddress, wethOsqfuPoolAddress, hedgeTimeThreshold, hedgePriceThreshold, auctionTime, minPriceMultiplier, maxPriceMultiplier)) as CrabStrategy;
                ...
                oSqfu.connect(d1).transfer(random.address, await oSqfu.balanceOf(d1.address))
                oSqfu.connect(d2).transfer(random.address, await oSqfu.balanceOf(d1.address))
                oSqfu.connect(d3).transfer(random.address, await oSqfu.balanceOf(d1.address))
            })
            ...
            it("migrate d2 with CR1 > CR2", async () => {
                ...
                const tx2 = await crabMigration.connect(d2).flashMigrateFromV1toV2(crabV1SharesBefore, 0, squfuryPoolFee);
                gasPaid = gasPaid.add(await getGasPaid(tx2))
            ...
            it("migrate d3 with CR1 < CR2", async () => {
                ...
                gasPaid = await getGasPaid(tx1)
                // Don't flash deposit and return back ETH. For the sake of simplicity. Flash deposit is already tested in previous tests
                const tx2 = await crabMigration.connect(d3).flashMigrateAndWithdrawFromV1toV2(crabV1SharesBefore, 0, ethToFlashLoan, maxEthToPay, squfuryPoolFee)
                gasPaid = gasPaid.add(await getGasPaid(tx2))
        ...
        describe("Migrate crab with odd supply", async function () {
            ...
            it("migrate d2 with CR1 > CR2 with flash deposit", async () => {
                ...
                const oSqfuToMintedFlash = wdiv(wmul(ethToFlashDeposit, expectedShort), expectedCollat);
                const ethToReturned = wmul(oSqfuToMintedFlash, oSqfuPrice)
                ...
                const tx2 = await crabMigration.connect(d2).flashMigrateFromV1toV2(crabV1SharesBefore, ethToFlashDeposit, squfuryPoolFee);
                gasPaid = gasPaid.add(await getGasPaid(tx2));
                ...
                expect(short).to.be.equal(expectedShort.add(oSqfuToMintedFlash))
                expect(crabV1SharesAfter).to.be.equal('0')
                expect(crabV1SharesInMigration).to.be.equal('0')
                expect(crabV2SharesBefore).to.be.equal('0')
        ...
        describe("Partial Migration", async function () {
            ...
            it("Partial migrate d2 with CR1 > CR2", async () => {
                ...
                const tx2 = await crabMigration.connect(d2).flashMigrateFromV1toV2(v1CrabToMigrate, 0, squfuryPoolFee);
                gasPaid = gasPaid.add(await getGasPaid(tx2))
            ...
            it("Partial migrate d2 with CR1 < CR2", async () => {
                ...
                await decreaseCR1(ethers.utils.parseEther('2'))
                const v1CrabToMigrate = crabV1SharesBefore.div(2);
                const [isFlashMigrate, ethNeededForV2, v1oSqfuToPay, ethToGetFromV1] = await crabMigration.flashMigrationDetails(v1CrabToMigrate)
                ...
                const expectedSqfuFromV2 = wdiv(wmul(ethToFlashLoan, shortV2), collatV2)
                const v1SqfuToFlashWithdraw = v1oSqfuToPay.sub(expectedSqfuFromV2)
                const maxEthToPay = wmul(v1SqfuToFlashWithdraw, oSqfuPrice).mul(1005).div(1000) // .5% slippage
                const ethToGetFromWithdraw = wdiv(wmul(expectedSqfuFromV2, collatV1), shortV1)
                const ethToGetFromFlashWithdraw = wdiv(wmul(v1SqfuToFlashWithdraw, collatV1), shortV1).sub(maxEthToPay)
                const expectedEth = ethToGetFromFlashWithdraw.add(ethToGetFromWithdraw).sub(ethToFlashLoan)
                ...
                gasPaid = await getGasPaid(tx1)
                // Don't flash deposit and return back ETH. For the sake of simplicity. Flash deposit is already tested in previous tests
                const tx2 = await crabMigration.connect(d2).flashMigrateAndWithdrawFromV1toV2(v1CrabToMigrate, 0, ethToFlashLoan, maxEthToPay, squfuryPoolFee)
                gasPaid = gasPaid.add(await getGasPaid(tx2))
    ...
    describe("User Flow Testing", () => {
        ...
        const deployAndSeedV1 = async () => {
            ...
            const p1 = oSqfu.connect(d1).transfer(random.address, await oSqfu.balanceOf(d1.address))
            const p2 = oSqfu.connect(d2).transfer(random.address, await oSqfu.balanceOf(d2.address))
            const p3 = oSqfu.connect(d3).transfer(random.address, await oSqfu.balanceOf(d3.address))
            const p4 = oSqfu.connect(d4).transfer(random.address, await oSqfu.balanceOf(d4.address))
            const p5 = oSqfu.connect(d5).transfer(random.address, await oSqfu.balanceOf(d5.address))
        ...
        describe("Case 1: batchMigrate() -> 100% withdraw by claim -> flashMigrateAndWithdraw -> flashMigrate", async () => {
            ...
            it("Should flashMigrateAndWithdraw", async () => {
                ...
                const tx2 = await crabMigration.connect(depositor).flashMigrateAndWithdrawFromV1toV2(crabV1SharesBefore, 0, ethToFlashLoan, maxEthToPay, squfuryPoolFee)
                gasPaid = gasPaid.add(await getGasPaid(tx2))
        ...
        describe("Case 5: batchMigrate() -> 100% ClaimAndWithdraw -> flashMigrateAndWithdraw -> flashMigrate", async () => {
            ...
            it("Should withdraw 100% via claimAndWithdraw", async () => {
                ...
                const d1SharesExpected = await crabMigration.sharesDeposited(d1.address)
                const d1Osqfu = wdiv(wmul(d1SharesExpected, shortV2), crabV2Supply)
                let oSqfuPrice = await oracle.getTwap(wethOsqfuPoolAddress, squfuryAddress, wethAddress, 1, false)
                const maxEthToPay = wmul(d1Osqfu, oSqfuPrice).mul(1005).div(1000) // .5% slippage
                ...
                oSqfuPrice = await oracle.getTwap(wethOsqfuPoolAddress, squfuryAddress, wethAddress, 1, false)
                const d2SharesExpected = await crabMigration.sharesDeposited(d2.address)
                const d2Osqfu = wdiv(wmul(d1SharesExpected, shortV2), crabV2Supply)
                const maxEthToPay2 = wmul(d2Osqfu, oSqfuPrice).mul(1005).div(1000) // .5% slippage
