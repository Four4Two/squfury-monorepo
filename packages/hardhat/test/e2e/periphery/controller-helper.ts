...
describe("ControllerHelper: mainnet fork", function () {
  ...
  this.beforeAll("Setup mainnet fork contracts", async () => {
    ...
    wSquFury = (await ethers.getContractAt("WPowerPerp", "0xf1B99e3E573A1a9C5E6B2Ce818b617F0E664E86B")) as WPowerPerp
    oracle = (await ethers.getContractAt("Oracle", "0x65D66c76447ccB45dAf1e8044e918fA786A483A1")) as Oracle
    shortSquFury = (await ethers.getContractAt("ShortPowerPerp", "0xa653e22A963ff0026292Cc8B67941c0ba7863a38")) as ShortPowerPerp
    wSquFuryPool = await ethers.getContractAt(POOL_ABI, "0x82c427AdFDf2d245Ec51D8046b41c4ee87F0d29C")
  ...
  describe("Flash mint short position, LP and use LP as collateral", async () => {
    it("open short, mint, LP oSQFU + ETH, deposit LP NFT and withdraw ETH collateral", async ()=> {
      const vaultId = (await shortSquFury.nextId());
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      const collateralToMint = debtInEth.mul(3).div(2).add(ethers.utils.parseUnits('0.01'))
      const squfuryPrice = await oracle.getTwap(wSquFuryPool.address, wSquFury.address, weth.address, 420, true)
      const collateralToLp = mintWSquFuryAmount.mul(squfuryPrice).div(one)
      const flashloanFee = collateralToMint.mul(9).div(1000)
      const flashloanWMintDepositNftParams = {
        wPowerPerpPool: wSquFuryPool.address,
        vaultId: 0,
        wPowerPerpAmount: mintWSquFuryAmount.toString(),
      ...
      const depositorSquFuryBalanceAfter = await wSquFury.balanceOf(depositor.address)
      ...
      expect(vaultAfter.shortAmount.sub(mintWSquFuryAmount).lte(1)).to.be.true
      expect(depositorSquFuryBalanceAfter.sub(depositorSquFuryBalanceBefore).lte(10)).to.be.true
      expect(vaultAfter.collateralAmount.eq(BigNumber.from(0))).to.be.true
    })

    it("open short, mint, LP oSQFU + ETH, deposit LP NFT and withdraw ETH collateral with too much oSQFU specified", async ()=> {
      // this is testing that the vault is minted with ~ the correct amount of oSQFU even if way too much is specified on f/e, which is what we want
      const vaultId = (await shortSquFury.nextId());
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      ...
      const flashloanWMintDepositNftParams = {
        wPowerPerpPool: wSquFuryPool.address,
        vaultId: 0,
        wPowerPerpAmount: mintWSquFuryAmount.mul(2).toString(),
    ...
    describe("open short in already overcollateralized vault, mint, LP oSQFU only, deposit LP NFT", async ()=> {
      before(async () => {
        const normFactor = await controller.getExpectedNormalizationFactor()
        const mintWSquFuryAmountToLp : BigNumber = ethers.utils.parseUnits('35')
        const mintRSquFuryAmount = mintWSquFuryAmountToLp.mul(normFactor).div(one)
        const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
        const scaledEthPrice = ethPrice.div(10000)
        const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
        const collateralAmount = debtInEth.mul(3)
  
        await controller.connect(depositor).mintWPowerPerpAmount(0, mintWSquFuryAmountToLp, 0, {value: collateralAmount})  
      })

      it("mint short, LP and use it as collateral", async () => {
        const vaultId = (await shortSquFury.nextId()).sub(1);
        await controller.connect(depositor).updateOperator(vaultId, controllerHelper.address);

        const depositorSquFuryBalanceBefore = await wSquFury.balanceOf(depositor.address)

        const mintWSquFuryAmount = ethers.utils.parseUnits('5')
        const collateralToMint = BigNumber.from(0)
        const collateralToLp = BigNumber.from(0)

        const slot0 = await wSquFuryPool.slot0()
        const currentTick = slot0[1]
  
        const isWethToken0 : boolean = parseInt(weth.address, 16) < parseInt(wSquFury.address, 16) 
        ...
        expect(BigNumber.from(vaultAfter.NftCollateralId).eq(tokenId)).to.be.true;
        expect(vaultAfter.shortAmount.sub(mintWSquFuryAmount.add(vaultBefore.shortAmount)).abs().lte(10)).to.be.true
        expect(depositorSquFuryBalanceAfter.sub(depositorSquFuryBalanceBefore).lte(10)).to.be.true
        expect(vaultAfter.collateralAmount.eq(vaultBefore.collateralAmount)).to.be.true
      })
    })

    it("open short in new vault, mint, LP oSQFU only, deposit LP NFT", async () => {
      const vaultId = (await shortSquFury.nextId());
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('100')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      ...
      expect(BigNumber.from(vaultAfter.NftCollateralId).eq(tokenId)).to.be.true;
      expect(vaultAfter.shortAmount.sub(mintWSquFuryAmount).abs().lte(10)).to.be.true      
      expect(depositorSquFuryBalanceAfter.sub(depositorSquFuryBalanceBefore).lte(10)).to.be.true
      expect(vaultAfter.collateralAmount.eq(collateralToMint.sub(collateralToFlashloan))).to.be.true
    })

    it("open short, mint with >0 ETH collateral, LP oSQFU + ETH, deposit LP NFT", async ()=> {
      const vaultId = (await shortSquFury.nextId());
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
    ...
    it("existing vault, mint with >0 ETH collateral, LP oSQFU + ETH, deposit LP NFT", async ()=> {
      // Make new vault with 2x collateral
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount : BigNumber = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      const collateralToDeposit = debtInEth.mul(2)
      await controller.connect(depositor).mintWPowerPerpAmount(0, mintWSquFuryAmount, 0, {value: collateralToDeposit})
      const vaultId = (await shortSquFury.nextId()).sub(1);
      await controller.connect(depositor).updateOperator(vaultId, controllerHelper.address)
      // Get before context
      const vaultBefore = await controller.vaults(vaultId)
      ...
      //expect(vaultAfter.shortAmount.sub(vaultBefore.shortAmount).sub(mintWSquFuryAmount).abs().lte(1000)).to.be.true
    ...
    it("open short in new vault, mint, LP oSQFU only, deposit LP NFT, no eth added", async () => {
      // New vault with 2x collateral
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount : BigNumber = ethers.utils.parseUnits('40')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      ...
      const flashLoanAmount = (vaultBefore.shortAmount.add(mintWSquFuryAmount)).mul(normFactor).mul(ethPrice).mul(3).div(one.mul(one).mul(10000).mul(2)).sub(vaultBefore.collateralAmount).add(ethers.utils.parseUnits('0.1'))

      const flashloanWMintDepositNftParams = {
        wPowerPerpPool: wSquFuryPool.address,
        vaultId: vaultId,
        wPowerPerpAmount: mintWSquFuryAmount,
      ...
      // Set up for one-sided LP with oSQFU only with half collateral flashloaned
      ...
      expect(ownerOfUniNFT === controller.address).to.be.true
      expect(vaultAfter.shortAmount.sub(vaultBefore.shortAmount).sub(mintWSquFuryAmount).abs().lte(10)).to.be.true
    ...
    it("open short in new vault, mint, LP oSQFU only, deposit LP NFT, some eth added", async () => {
      // New vault with 1.5x collateral
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount : BigNumber = ethers.utils.parseUnits('100')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      ...
      expect(ownerOfUniNFT === controller.address).to.be.true
      expect(vaultAfter.shortAmount.sub(mintWSquFuryAmount.mul(2)).abs().lte(10)).to.be.true
  ...
  describe("Close LP position in vault: LP have more wPowerPerp than needed amount to burn", async () => {
    before("open first short position and LP" , async () => {
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmountToLp : BigNumber = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmountToLp.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      ...
      const squfuryPrice = await oracle.getTwap(wSquFuryPool.address, wSquFury.address, weth.address, 420, true)
      const collateralToLp = mintWSquFuryAmountToLp.mul(squfuryPrice).div(one)
      ...
      const mintParam = {
        ...
        amount0Desired: isWethToken0 ? collateralToLp : mintWSquFuryAmountToLp,
        amount1Desired: isWethToken0 ? mintWSquFuryAmountToLp : collateralToLp,
        amount0Min: 0,
        amount1Min: 0,
      ...
      await wSquFury.connect(depositor).approve(positionManager.address, ethers.constants.MaxUint256)  
    ...
    before("open short amount more than amount in LP position" , async () => {
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('35')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
      ...
      await controller.connect(depositor).mintWPowerPerpAmount(0, mintWSquFuryAmount, tokenId)
    })
    ...
    it("close LP position deposited as collateral in vault", async () => {
      ...
      const limitPriceEthPerPowerPerp = squfuryPrice.mul(one.sub(slippage)).div(one);
  ...
  describe("Close LP position in vault: LP have less wPowerPerp than needed amount to burn", async () => {
    ...
    before("open short amount more than amount in LP position" , async () => {
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('55')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
      const scaledEthPrice = ethPrice.div(10000)
      const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
    ...
    it("close LP position deposited as collateral in vault", async () => {
      ...
      const squfuryPrice = await oracle.getTwap(wSquFuryPool.address, wSquFury.address, weth.address, 420, true)
      const slippage = BigNumber.from(3).mul(BigNumber.from(10).pow(16))
      const limitPriceEthPerPowerPerp = squfuryPrice.mul(one.add(slippage)).div(one);
      const positionBefore = await (positionManager as INonfungiblePositionManager).positions(uniTokenId);
      const flashloanCloseVaultLpNftParam = {
        vaultId: vaultId,
        tokenId: uniTokenId,
  ...
  describe("Collect fees from vault and redeposit", async () => {
    before("open short position and LP" , async () => {
      ...
      const squfuryPrice = await oracle.getTwap(wSquFuryPool.address, wSquFury.address, weth.address, 1, true)
      const collateralToLp = mintWSquFuryAmount.mul(squfuryPrice).div(one)
      const flashloanWMintDepositNftParams = {
        wPowerPerpPool: wSquFuryPool.address,
        vaultId: 0,
    ...
    it("swap with pool, collect fees and redeposit uni nft in vault", async () => {
      ...
      const ownerSquFuryBalanceBeforeTrade1 = await wSquFury.balanceOf(owner.address)
      ...
      const ownerSquFuryBalanceAfterTrade1 = await wSquFury.balanceOf(owner.address)
      ...
      // 
      const wSquFuryToSell = ownerSquFuryBalanceAfterTrade1.sub(ownerSquFuryBalanceBeforeTrade1).sub(ethers.utils.parseUnits('0.01'))
      const swapParamSell = {
        ...
        recipient: owner.address,
        deadline: Math.floor(await getNow(ethers.provider) + 8640000),
        amountIn: wSquFuryToSell.toString(),
        amountOutMinimum: 0,
        sqrtPriceLimitX96: 0
      }    

      const ownerSquFuryBalanceBeforeTrade2 = await wSquFury.balanceOf(owner.address)
      await wSquFury.connect(owner).approve(uniswapRouter.address, constants.MaxUint256)
      await uniswapRouter.connect(owner).exactInputSingle(swapParamSell)
      const ownerSquFuryBalanceAfterTrade2 = await wSquFury.balanceOf(owner.address)
      expect(ownerSquFuryBalanceBeforeTrade2.sub(ownerSquFuryBalanceAfterTrade2).eq(wSquFuryToSell)).to.be.true
      ...
      expect(depositorSquFuryBalanceAfter.gt(depositorSquFuryBalanceBefore)).to.be.true
  ...
  describe("Rebalance with vault", async () => {
    ...
    it("Close vault LP and open new LP with the a different range, same amount of ETH and oSQFU", async () => {
      ...
      await shortSquFury.connect(depositor).approve(controller.address, vaultId);
      await controller.connect(depositor).updateOperator(vaultId, controllerHelper.address)
      ...
      const rebalanceLpInVaultParams = [
        {
         ...
         {
           // Mint new LP (add 0.01 oSQFU noise since exact value will not be known)
           ...
           data: abiCoder.encode(["address", "address", "uint256", 'uint256','uint256', 'uint256', 'uint256', 'uint256', 'int24', 'int24'],
            [controllerHelper.address, wSquFuryPool.address, vaultId, wPowerPerpAmountInLPBefore.add(surpriseProceeds), BigNumber.from(0), wethAmountInLPBefore, amount0Min, amount1Min, newTickLower, newTickUpper])
         }
      ]
      // Flashloan to cover complete removal of LP (rearrange collateral ratio formula for 1.5 and add 0.1 ETH safety margin)
      const normFactor = await controller.getExpectedNormalizationFactor()
      ...
      const depositorSquFuryDiff = depositorSquFuryBalanceAfter.sub(depositorSquFuryBalanceBefore)
      const vaultEthDiff = vaultAfter.collateralAmount.sub(vaultBefore.collateralAmount)
      const vaultSquFuryDiff = vaultAfter.shortAmount.sub(vaultBefore.shortAmount)
      const lpEthDiff = wethAmountInLPAfter.sub(wethAmountInLPBefore)
      const lpSquFuryDiff = wPowerPerpAmountInLPAfter.sub(wPowerPerpAmountInLPBefore)
      // Assertions
      ...
      // SquFury preserving
      expect(depositorSquFuryDiff.sub(vaultSquFuryDiff).add(lpSquFuryDiff).abs().lte(10)).to.be.true
    })
  })
  ...
  it("Close vault LP and open new one-sided LP with just eth ", async () => {
    ...
    const amount0Min = BigNumber.from(0);
    const amount1Min = BigNumber.from(0);
    // Estimate proceeds from liquidating squfury in LP
    const ethAmountOutFromSwap = await quoter.connect(depositor).callStatic.quoteExactInputSingle(wSquFury.address,
      weth.address,
      3000,
    ...
    const rebalanceLpInVaultParams = [
      {
        ...
        rebalanceLpInVaultType: BigNumber.from(5), // GeneralSwap:
          ...
          data: abiCoder.encode(["address", 'address', 'uint256', 'uint256', 'uint24'],
           [wSquFury.address, weth.address, wPowerPerpAmountInLPBefore.sub(safetyWPowerPerp), BigNumber.from(0), 3000])
          },
       {
         // Mint new LP 
         rebalanceLpInVaultType:  BigNumber.from(4), // MintNewLP
         ...
         data: abiCoder.encode(["address", "address", "uint256", 'uint256','uint256', 'uint256', 'uint256', 'uint256', 'int24', 'int24'],
          [controllerHelper.address, wSquFuryPool.address, vaultId, BigNumber.from(0), BigNumber.from(0), wethAmountToLP, amount0Min, amount1Min, newTickLower, newTickUpper])
       }
    ]
...
it("Mint more oSQFU, withdraw half eth from LP, deposit some eth into vault and withdraw some eth ", async () => {
  // Get vault and LP info
  const depositorSquFuryBalanceBefore = await wSquFury.balanceOf(depositor.address)
  const depositorEthBalanceBefore = await ethers.provider.getBalance(depositor.address)
  const vaultId = (await shortSquFury.nextId()).sub(1);
  ...
  // Assertions
  expect(vaultSquFuryDiff.eq(wPowerPerpAmountToMint)).to.be.true
  expect(vaultEthDiff.eq(wethAmountToDeposit)).to.be.true
  expect(lpSquFuryDiff.add(wPowerPerpAmountInLPBefore.div(2)).lte(10)).to.be.true
  expect(lpEthDiff.add(wethAmountInLPBefore.div(2)).lte(10)).to.be.true
  expect(depositorSquFuryDiff.sub(wPowerPerpAmountInLPBefore.div(2)).sub(wPowerPerpAmountToMint).lte(10)).to.be.true 
})
...
it("Mint more oSQFU, withdraw half eth from LP, deposit some eth into vault and withdraw some eth ", async () => {
  ...
  expect(lpSquFuryDiff.add(wPowerPerpAmountInLPBefore.div(2)).lte(10)).to.be.true
  expect(lpEthDiff.add(wethAmountInLPBefore.div(2)).lte(10)).to.be.true
  expect(depositorSquFuryDiff.sub(wPowerPerpAmountInLPBefore.div(2)).sub(wPowerPerpAmountToMint).abs().lte(10) ) 
})

})

describe("Rebalance LP in vault to just oSQFU", async () => {
  before("Mint new full range LP outside of vault" , async () => {
    // Mint 50 squfury in new vault
    const normFactor = await controller.getExpectedNormalizationFactor()
    const mintWSquFuryAmount = ethers.utils.parseUnits('50')
...
it("Close vault LP and open new one-sided LP with just oSQFU ", async () => {
  // Get vault and LP info
  const depositorSquFuryBalanceBefore = await wSquFury.balanceOf(depositor.address)
  const depositorEthBalanceBefore = await ethers.provider.getBalance(depositor.address)
  const vaultId = (await shortSquFury.nextId()).sub(1);
  ...
  const rebalanceLpInVaultParams = [
    {
      ...
      {
      // Sell all weth for oSQFU
      rebalanceLpInVaultType: BigNumber.from(5), // GeneralSwap:
        // GeneralSwap: [tokenIn, tokenOut, amountIn, limitPrice, poolFee]
        data: abiCoder.encode(["address", 'address', 'uint256', 'uint256','uint24'],
         [weth.address, wSquFury.address, wethAmountInLPBefore, BigNumber.from(0), 3000])
        },
     {
       // Mint new LP 
       rebalanceLpInVaultType:  BigNumber.from(4), // MintNewLP
       // lpWPowerPerpPool: [recipient, wPowerPerpPool, vaultId, wPowerPerpAmount, collateralToDeposit, collateralToLP, amount0Min, amount1Min, lowerTick, upperTick ]
       data: abiCoder.encode(["address", "address", "uint256", 'uint256','uint256', 'uint256', 'uint256', 'uint256', 'int24', 'int24'],
        [controllerHelper.address, wSquFuryPool.address, vaultId, wPowerPerpAmountToLp, BigNumber.from(0),  BigNumber.from(0), amount0Min, amount1Min, newTickLower, newTickUpper])
     }
  ]
  // Flashloan to cover complete removal of LP (rearrange collateral ratio formula for 1.5 and add 0.1 ETH safety margin)
  const normFactor = await controller.getExpectedNormalizationFactor()
  ...
  describe("Increase liquidity in vault with no eth added", async () => {
    before("Mint new full range LP outside of vault" , async () => {
      // Mint 50 squfury in new vault with 2x collateral ratio
      const normFactor = await controller.getExpectedNormalizationFactor()
      const mintWSquFuryAmount = ethers.utils.parseUnits('50')
      const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
      const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
  ...
  it("Deposit nft, withdraw collateral from vault, mint, increase liquidity with no eth added", async () => {
    ...
    // - Mint some squfury (DepositIntoVault)
    ...
    const rebalanceLpInVaultParams = [
        ...
        },
      {
        ...
        data: abiCoder.encode(["address","uint256", 'uint256', 'uint256', 'uint256', 'uint256','uint256'],
         [wSquFuryPool.address, tokenId, wPowerPerpAmountToLp, BigNumber.from(0), wethAmountToLp, BigNumber.from(0), BigNumber.from(0)])
        }
    ]
    // Flashloan to cover complete removal of LP (rearrange collateral ratio formula for 1.5 and add 0.1 ETH safety margin)
    const normFactor = await controller.getExpectedNormalizationFactor()
    ...
    expect(flashLoanAmount.gte(0)).to.be.true
    // expect(positionAfter.tickLower === newTickLower).to.be.true
    expect(vaultSquFuryDiff.sub(wPowerPerpAmountToLp).eq(0)).to.be.true
    // LP eth comes from vault
    expect(vaultEthDiff.add(lpEthDiff).lte(10)).to.be.true
    // Nft id unchanged
    expect(vaultAfter.NftCollateralId==vaultBefore.NftCollateralId).to.be.true
    // SquFury convervation
    expect(lpSquFuryDiff.sub(vaultSquFuryDiff).add(depositorSquFuryDiff).abs().lte(10)).to.be.true
    // LP amount close to mint
    expect(lpSquFuryDiff.sub(vaultSquFuryDiff).lte(ethers.utils.parseUnits('0.1'))).to.be.true
    })
    })


    describe("Rebalance LP in vault: reduce oSQFU < debt, burn, withdraw >0 eth", async () => {
      before("Mint new full range LP outside of vault" , async () => {
        // Mint 50 squfury in new vault
        const normFactor = await controller.getExpectedNormalizationFactor()
        const mintWSquFuryAmount = ethers.utils.parseUnits('50')
    ...
    it("Reduce liquidity in LP, swap oSQFU for eth, deposit eth into vault, mint more oSQFU", async () => {
      // Get vault and LP info
      const depositorSquFuryBalanceBefore = await wSquFury.balanceOf(depositor.address)
      const depositorEthBalanceBefore = await ethers.provider.getBalance(depositor.address)
      const vaultId = (await shortSquFury.nextId()).sub(1);
      ...
      // Estimate eth amount needed to buy remaining oSQFU
      ...
      const rebalanceLpInVaultParams = [
        {
          ...
          },
          {
            ...
            // GeneralSwap: [tokenIn, tokenOut, amountIn, limitPrice, poolFee]
            data: abiCoder.encode(["address", 'address', 'uint256', 'uint256','uint24'],
            [wSquFury.address, weth.address,  wPowerPerpToSwap, BigNumber.from(0), 3000])
          }  ,
          {
      ...
      expect(vaultSquFuryDiff.sub(wPowerPerpToSwap).eq(0)).to.be.true
      ...
      describe("Rebalance LP in vault: reduce oSQFU > debt, burn, withdraw >0 eth", async () => {
        ...
        before("Flashmint new LP with 200 oSQFU and withdraw all eth collateral", async () => {
          const normFactor = await controller.getExpectedNormalizationFactor()
          const mintWSquFuryAmount = ethers.utils.parseUnits('200')
          const mintRSquFuryAmount = mintWSquFuryAmount.mul(normFactor).div(one)
          const ethPrice = await oracle.getTwap(ethUsdcPool.address, weth.address, usdc.address, 420, true)
          const scaledEthPrice = ethPrice.div(10000)
          const debtInEth = mintRSquFuryAmount.mul(scaledEthPrice).div(one)
          ...
          const wPowerPerpToBurn = mintWSquFuryAmount.div(2)
          const vaultId = (await shortSquFury.nextId()).sub(1);
      ...
      it("Withdraw oSQFU > vault debt from LP, burn, swap remainder, withdraw >0 eth", async () => {
        // Get vault and LP info
        const depositorSquFuryBalanceBefore = await wSquFury.balanceOf(depositor.address)
        const depositorEthBalanceBefore = await ethers.provider.getBalance(depositor.address)
        const vaultId = (await shortSquFury.nextId()).sub(1);
        ...
        const rebalanceLpInVaultParams = [
          ...
          {
            // Sell all oSQFU for weth
            rebalanceLpInVaultType: BigNumber.from(5), // GeneralSwap:
            // GeneralSwap: [tokenIn, tokenOut, amountIn, limitPrice, poolFee]
            data: abiCoder.encode(["address", 'address', 'uint256', 'uint256','uint24'],
            [wSquFury.address, weth.address, wPowerPerpAmountToSwap, BigNumber.from(0), 3000])
          }  
        ]
        // Flashloan to cover complete removal of LP (rearrange collateral ratio formula for 1.5 and add 0.1 ETH safety margin)
        const normFactor = await controller.getExpectedNormalizationFactor()
        ...
        const lpSquFuryDiff = wPowerPerpAmountInLPAfter.sub(wPowerPerpAmountInLPBefore)
        // Assertions
        expect(vaultSquFuryDiff.add(wPowerPerpToBurn).eq(0)).to.be.true
        // LP eth comes from vault
      ...
      it("Deposit nft, withdraw collateral from vault, mint, increase liquidity with some eth added", async () => {
        ...
        // console.log('vaultSquFuryDiff',vaultSquFuryDiff.toString() )
        // console.log('vaultEthDiff',vaultEthDiff.toString() )
        // console.log('lpSquFuryDiff', lpSquFuryDiff.toString())
        // console.log('lpEthDiff', lpEthDiff.toString())
        // console.log('depositorSquFuryDiff', depositorSquFuryDiff.toString())
        ...
        // Target squfury minted
        expect((vaultSquFuryDiff.sub(wPowerPerpAmountToLp)).abs().lte(10)).to.be.true
        // Nft id unchanged
        expect(vaultAfter.NftCollateralId==vaultBefore.NftCollateralId).to.be.true
          })
        })
